...
import ScrollText2 from "@/components/animations/ScrollText2";
...
// Usage:
// <ScrollText2 text="${text}" duration={${duration}} stagger={${stagger}} blurAmount={${blurAmount}} splitBy="${splitBy}" />`;


const ScrollText2Page = () => {
  const [text, setText] = useState("Scroll down to reveal this blurred animated text");
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-4 sm:px-6 md:px-8 pt-6 md:pt-8 pb-4">
        ...
        <h1 className="text-2xl font-bold text-foreground">Scroll Text 2</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Scroll-triggered text reveal with a blur-to-sharp effect alongside fade and slide transitions.
        </p>
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-6">
        {view === "preview" ? (
          ...
          <CodeBlock code={generateCode(text, duration, stagger, splitBy, blurAmount, color, fontSize)} language="React / TypeScript" />
        )}
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-8">
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Blur Amount <span className="text-primary ml-1">{blurAmount}px</span></Label>
              <Slider value={[blurAmount]} onValueChange={([v]) => { setBlurAmount(v); resetPreview(); }} min={2} max={20} step={1} />
            </div>
            <div className="space-y-2">
...
export default ScrollText2Page;
