...
import LiquidVenom from "@/components/animations/LiquidVenom";
...
const generateCode = (speed: number, flashIntensity: number, size: number) =>
...
// Usage:
// <LiquidVenom speed={${speed}} flashIntensity={${flashIntensity}} width={${size}} height={${size}} />`;


const LiquidVenomPage = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-4 sm:px-6 md:px-8 pt-6 md:pt-8 pb-4">
        ...
        <h1 className="text-2xl font-bold text-foreground">Liquid Venom</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Dual metallic ribbons with satin flash highlights and cinematic vignette — dark, luxurious, and mesmerizing.
        </p>
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-6">
        {view === "preview" ? (
          <PreviewFrame>
            <LiquidVenom key={previewKey} speed={speed} flashIntensity={flashIntensity} width={size} height={size} />
          </PreviewFrame>
        ) : (
          <CodeBlock code={generateCode(speed, flashIntensity, size)} language="React / TypeScript" />
        )}
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-8">
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Flash Intensity <span className="text-primary ml-1">{flashIntensity.toFixed(1)}</span></Label>
              <Slider value={[flashIntensity]} onValueChange={([v]) => { setFlashIntensity(v); resetPreview(); }} min={0} max={1} step={0.05} />
            </div>
            <div className="space-y-2">
...
export default LiquidVenomPage;
