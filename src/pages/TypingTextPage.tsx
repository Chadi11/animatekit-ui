...
const generateCode = (text: string, speed: number, delay: number, loop: boolean, fontSize: number, color: string) =>
...
// Usage:
// <TypingText text="${text}" speed={${speed}} loop={${loop}} />`;
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-4 sm:px-6 md:px-8 pt-6 md:pt-8 pb-4">
        ...
        <h1 className="text-2xl font-bold text-foreground">Typing Text</h1>
        <p className="text-sm text-muted-foreground mt-1">A typewriter effect that reveals text character by character, no cursor.</p>
      </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-6">
        {view === "preview" ? (
          <PreviewFrame>
            <TypingText key={previewKey} text={text} speed={speed} delay={delay} loop={loop} fontSize={fontSize} color={color} />
          </PreviewFrame>
        ) : (
          <CodeBlock code={generateCode(text, speed, delay, loop, fontSize, color)} language="React / TypeScript" />
        )}
      </div>
